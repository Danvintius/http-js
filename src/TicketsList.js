      ...
      if (targetClass.startsWith('ticket__control-status')) {
        ...
        this.negotiator.createRequest({
          ...
          callback: (response) => {
            ...
            console.log(`Статус тикета с id #${ticket.id} был изменен на ${receivedData.status}`); // eslint-disable-line no-console
          },
        });
  ...
  downloadTickets() {
    this.negotiator.createRequest({
      ...
      callback: (response) => {
        ...
        console.log('Все тикеты загружены с сервера.'); // eslint-disable-line no-console
      },
    });
  }
}